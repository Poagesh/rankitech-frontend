...
function UserDashboard() {
  ...
  const handleUserAction = (action) => {
    switch (action) {
      case 'profile':
        ...
        break;
      case 'logout':
        // Clear any auth tokens or user data (implement your auth logic here)
  ...
  const handleResumeUpload = async (event) => {
    ...
    try {
      ...
      setUploadError('');
      // Optionally, refresh job matches after upload
      // Example: Fetch updated matches from API
      // setMatchedJobs(response.data.updatedMatches || matchedJobs);
    } catch (error) {
                ...
                >
                  <div className="d-flex justify-content-between align-items-start mb-3">
                    ...
                    <div
                      style={{
                        ...styles.matchScore,
                        background: `linear-gradient(135deg, ${getScoreColor(job.matchScore)}, ${getScoreColor(
                          job.matchScore
                        )}dd)`,
                      }}
                    >
